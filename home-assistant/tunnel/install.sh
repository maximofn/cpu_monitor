#!/usr/bin/env bash
# Instala el túnel SSH forward (raspihome → wallabot, puerto 9124) como user
# systemd unit. Diseñado para correr EN raspihome.
#
# Mantiene `ssh -L 9124:127.0.0.1:9124 wallabot` vivo con auto-reconnect.
# Como HA corre con --network host, leer 127.0.0.1:9124 desde el contenedor
# llega al cpu-monitord remoto.
#
# Requisitos previos:
...
UNIT_NAME="cpu-monitor-ha-tunnel.service"
UNIT_DIR="${XDG_CONFIG_HOME:-$HOME/.config}/systemd/user"
SRC_UNIT="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/$UNIT_NAME"
TUNNEL_KEY="$HOME/.ssh/id_ed25519_cpu_tunnel"
...
case "${1:-install}" in
    install)
        ...
        if [ ! -f "$TUNNEL_KEY" ]; then
            ...
            ssh-keygen -t ed25519 -N "" -f "$TUNNEL_KEY" -C "raspihome-cpu-tunnel" >/dev/null
            echo
