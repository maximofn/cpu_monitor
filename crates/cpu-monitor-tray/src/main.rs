...
use tray::{CpuTray, State};

const ASSET_ICON_REL: &str = "assets/cpu.png";
...
async fn main() -> Result<()> {
    ...
    let icon_path = locate_icon().context("could not find cpu.png; install via `assets/`")?;
    ...
    let tray_state = CpuTray::new(renderer, cfg.backend_url.clone(), cache_dir)?;
...
async fn dump_icon_once(
    ...
    let snapshot: cpu_monitor_core::Snapshot = reqwest::get(&url)
        .await
    ...
    let rendered = renderer.render(Some(&snapshot.cpu), true);
...
fn cache_icon_dir() -> Result<PathBuf> {
    ...
    Ok(base.join("cpu-monitor").join("icons"))
}

fn locate_icon() -> Option<PathBuf> {
    let mut candidates: Vec<PathBuf> = Vec::new();
    if let Some(path) = std::env::var_os("CPU_MONITOR_TRAY_ICON") {
    ...
    if let Some(data) = xdg_data {
        candidates.push(data.join("cpu-monitor").join("cpu.png"));
    }
    candidates.push(PathBuf::from("/usr/share/cpu-monitor/cpu.png"));
