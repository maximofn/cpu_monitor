...
struct Config: Sendable {
    ...
    static let `default` = Config(
        backendURL: ProcessInfo.processInfo.environment["CPU_MONITOR_TRAY_URL"]
            ?? "http://127.0.0.1:9124",
        iconHeight: 22,
        logLevel: "info",
        dumpIcon: nil
    ...
    static func parse(_ args: [String]) -> Config {
        ...
        while i < args.count {
            ...
            case "-h", "--help":
                ...
                exit(0)
            case "--version":
                print("cpu-monitor-tray-mac 2.0.0-alpha.1")
                exit(0)
            default:
    ...
    static func printUsage() {
        let usage = """
        cpu-monitor-tray-mac — macOS menu bar frontend for cpu-monitord

        USAGE:
            cpu-monitor-tray-mac [OPTIONS]

        OPTIONS:
            --backend-url <URL>     Base URL of cpu-monitord (default: http://127.0.0.1:9124,
                                    env: CPU_MONITOR_TRAY_URL)
